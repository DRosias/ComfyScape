...
class ServerConstants {
    companion object {
        /** Refuse to start unless online-production security invariants are satisfied. */
        @JvmField
        var PRODUCTION_MODE: Boolean = true

        /** Normalized usernames authorized to hold owner or moderator database rights. */
        @JvmField
        var OWNER_ACCOUNTS: Set<String> = emptySet()

        @JvmField
        var MODERATOR_ACCOUNTS: Set<String> = emptySet()
        ...
        /** Credits assigned to newly created accounts. */
        @JvmField
        var STARTING_CREDITS = 2000
        ...
        //the server name
        @JvmField
        var SERVER_NAME: String = "ComfyScape"

        //the server's grand exchange name
        @JvmField
        var SERVER_GE_NAME: String = "ComfyScape"

        //The RSA_KEY for the server.
        @JvmField
        var EXPONENT = BigInteger("")

        //The MODULUS for the server.
        @JvmField
        var MODULUS = BigInteger("")
